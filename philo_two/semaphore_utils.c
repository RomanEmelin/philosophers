...
/*   Updated: 2021/03/26 18:18:23 by mwinter          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int			get_block(t_philo *philo)
{
	...
	philo->args->s_block = (sem_t **)malloc(sizeof(sem_t *) *
			philo->args->philo_cnt);
	if (!philo->args->s_block)
		return (1);
	i = -1;
	while (++i < philo->args->philo_cnt)
	{
		if (!(name = get_name(i + 1)))
			return (1);
		sem_unlink(name);
		philo->args->s_block[i] = sem_open(name, O_CREAT, 0644, 1);
		if (philo->args->s_block[i] == SEM_FAILED)
			return (1);
		free(name);
	}
	return (0);
...
void		close_block(t_philo *philo)
{
	...
	while (++i < philo->args->philo_cnt)
	{
		name = get_name(i + 1);
		sem_unlink(name);
		sem_close(philo->args->s_block[i]);
		free(name);
	}
	free(philo->args->s_block);
}
